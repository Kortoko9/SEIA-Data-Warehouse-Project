# brz_toast_config_tip_withholding

Artifact Type: Power Query M
Build Method: Query
Dependencies: Toast auth; restaurant GUID; config:read scope
Grain: One row per raw config payload
Last Updated: April 27, 2026
Layer: Bronze
Notes: Restaurant-level tip withholding config.
Primary / Merge Key: payload_hash
Purpose: Pull tip withholding config into Bronze.
Source Objects: Toast API /config/v2/tipWithholding
Status: Documented
Target Object: brz_toast_config_tip_withholding
...
let
    ...
    sourceSystem = "toast",
    sourceEndpoint = "/config/v2/tipWithholding",
    recordSource = "config_tip_withholding_api",
    ...
    response =
        Json.Document(
            Web.Contents(
                apiHost,
                [
                    RelativePath = "config/v2/tipWithholding",
                    Headers = [
