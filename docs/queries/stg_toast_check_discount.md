...
Grain: One row per check-level discount application
Last Updated: April 27, 2026
Layer: Silver
Notes: Uses config fallback for discount type/value.
Primary / Merge Key: restaurant_guid + check_guid + discount_guid + ordinal
Purpose: Normalize check-level applied discounts.
Source Objects: brz_toast_orders_bulk; brz_toast_config_discounts
Status: Documented
Target Object: stg_toast_check_discount
...
let
    ...
    AddDiscountList =
        Table.AddColumn(
            KeepChecksOnly,
            "_discount_list",
            each GetListField([_check], {"appliedDiscounts", "discounts"}),
            type list
        ),
