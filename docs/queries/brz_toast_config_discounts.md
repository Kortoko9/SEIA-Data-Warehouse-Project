...
Layer: Bronze
Notes: Used by discount staging tables.
Primary / Merge Key: payload_hash
Purpose: Pull discount config into Bronze.
Source Objects: Toast API /config/v2/discounts
Status: Documented
Target Object: brz_toast_config_discounts
...
let
    ...
    sourceSystem = "toast",
    sourceEndpoint = "/config/v2/discounts",
    recordSource = "config_discounts_api",
    ...
    response =
        Json.Document(
            Web.Contents(
                apiHost,
                [
                    RelativePath = "config/v2/discounts",
                    Headers = [
