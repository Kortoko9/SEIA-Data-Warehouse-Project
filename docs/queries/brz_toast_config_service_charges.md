# brz_toast_config_service_charges
...
Layer: Bronze
Notes: Used by service charge staging table.
Primary / Merge Key: payload_hash
Purpose: Pull service charge config into Bronze.
Source Objects: Toast API /config/v2/serviceCharges
Status: Documented
Target Object: brz_toast_config_service_charges
...
let
    ...
    sourceSystem = "toast",
    sourceEndpoint = "/config/v2/serviceCharges",
    recordSource = "config_service_charges_api",
    ...
    response =
        Json.Document(
            Web.Contents(
                apiHost,
                [
                    RelativePath = "config/v2/serviceCharges",
                    Headers = [
