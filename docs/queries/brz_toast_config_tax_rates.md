...
Layer: Bronze
Notes: Used by selection tax staging table.
Primary / Merge Key: payload_hash
Purpose: Pull tax rate config into Bronze.
Source Objects: Toast API /config/v2/taxRates
Status: Documented
Target Object: brz_toast_config_tax_rates
...
let
    ...
    sourceSystem = "toast",
    sourceEndpoint = "/config/v2/taxRates",
    recordSource = "config_tax_rates_api",
    ...
    response =
        Json.Document(
            Web.Contents(
                apiHost,
                [
                    RelativePath = "config/v2/taxRates",
                    Headers = [
