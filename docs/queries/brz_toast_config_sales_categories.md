# brz_toast_config_sales_categories
...
Layer: Bronze
Notes: Config dimension source.
Primary / Merge Key: payload_hash
Purpose: Pull sales category config into Bronze.
Source Objects: Toast API /config/v2/salesCategories
Status: Documented
Target Object: brz_toast_config_sales_categories
...
let
    ...
    sourceSystem = "toast",
    sourceEndpoint = "/config/v2/salesCategories",
    recordSource = "config_sales_categories_api",
    ...
    response =
        Json.Document(
            Web.Contents(
                apiHost,
                [
                    RelativePath = "config/v2/salesCategories",
                    Headers = [
